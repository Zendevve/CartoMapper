--[[--------------------------------------------------------------------------------------------
FogClear.lua
Unexplored map reveal module.
Uses Mapster's bit-packed coordinate database.
--------------------------------------------------------------------------------------------]]--
...
local function UpdateOverlayTextures(frame, frameName, scale, alphaMod)
    ...
    if not overlayMap then return end

    local numOverlays = GetNumMapOverlays()
    local pathLen = string.len(pathPrefix) + 1

    -- Fetch actually discovered overlays from WoW API
    for i=1, numOverlays do
        local texName, texWidth, texHeight, offsetX, offsetY = GetMapOverlayInfo(i)
        if texName and texName ~= "" then
            texName = string.sub(texName, pathLen)
            local texID = texWidth + texHeight * 2^10 + offsetX * 2^20 + offsetY * 2^30
            if texID ~= 0 and texID ~= 131200 and string.lower(texName) ~= "pixelfix" then
    ...
    -- Loop over ALL overlays (both discovered and undiscovered in our database)
    for texName, texID in pairs(overlayMap) do
        local textureName = pathPrefix .. texName
        local textureWidth = math.fmod(texID, 2^10)
        local textureHeight = math.fmod(math.floor(texID / 2^10), 2^10)
        local offsetX = math.fmod(math.floor(texID / 2^20), 2^10)
        local offsetY = math.floor(texID / 2^30)

        local numTexturesWide = math.ceil(textureWidth / 256)
        local numTexturesTall = math.ceil(textureHeight / 256)
        local neededTextures = textureCount + (numTexturesWide * numTexturesTall)

        -- Dynamically create texture frames if needed
        if neededTextures > numOv then
            for j = numOv + 1, neededTextures do
                frame:CreateTexture(string.format(frameName, j), "ARTWORK")
            end
        ...
        for j = 1, numTexturesTall do
            ...
            for k = 1, numTexturesWide do
                ...
                local texture = _G[string.format(frameName, textureCount)]
                ...
                texture:SetTexture(string.format(textureName.."%d", ((j - 1) * numTexturesWide) + k))

                -- Style: Discovered vs Undiscovered
                if discoveredOverlays[texName] then
                    ...
                    texture:SetAlpha(alpha * (1 - (alphaMod or 0)))
    ...
    -- Hide remaining unused texture overlays
    for i = textureCount + 1, numOv do
        local tex = _G[string.format(frameName, i)]
...
local function UpdateBattlefieldMinimapOverlays()
